...
10. Definimos la clase de la celda en el storyboard con la nueva clase creada (HeroTableViewCell).
11. Conectamos la imagen y la label del sotryboard a la clase de la cell para que se creen las variables.
1
13. self.tableView.reloadData() recarga los datos de la tabla. Esta función deberá llamarse cuando se hayan cargado los datos a través de la peticion http.
...
extension HomeViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
	    // abrimos la HeroDetailViewController al hacer click sobre la cell
		guard let viewController = self.storyboard?.instantiateViewController(withIdentifier: "HeroDetailViewController") as? HeroDetailViewController else {
			return
		}

		viewController.hero = self.heroes[indexPath.row]
    }
}
...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
	...
	    guard let cell = tableView.dequeueReusableCell(withIdentifier: "HeroTableViewCell", for: indexPath) as? HeroTableViewCell else {
		    // obligamos a que crashee un error
		    fatalError()
	    }
...
        cell.nameLabel.text = hero.name
		cell.avatarImageView.image = UIImage(named: hero.photo)
...
### Crear la pantalla de detalle  del hero
1. Creamos una nuevaq CocoaTouchClass y creamos un nuevo ViewController "HeroDetailViewController"
2. Creamos un nuevo view controller en el storyboard y especificamos que su clase es HeroDetailViewController.
3. Añadimos una ImageView que tenga 0 en todas las restricciones (para que ocupe toda la pantalla).
4. Arrastramos la ImageView hasta la clase HeroDetailViewController .
5. 
...
class HeroDetailViewController : UIViewController {
	...
	override func viewDidLoad() {
		...
		self.avatarImageView.image = UIImage(named: hero.photo)
	}
}
```
