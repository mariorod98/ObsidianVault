...
Esta keyword especifica que una variable es constante en tiempo de compilación

```cpp
int bla = 12 * 3; // el compilador calcula la multiplicación
int bla2 = bla + 3; // aqui no lo hace

constexpr int bla = 12 * 3; // el compilador calcula la multiplicación
int bla2 = bla + 3;         // ahora sí lo calcula
```

Similar a constexpr pero para funciones. Esa función se realizará en compilación y no existirá en el código fuente
...
[[nodiscard]] consteval division(constexpr int a, constexpr int b) {
	if constexpr(b == 0) {
		// se usa para definir que la función nunca debería llegar aquí. Mostrará un mensaje de error si es el caso. De este modo, se avisa al programador de que se ha alcanzado ese código y no debería poderse.
		std::unreachable()
	}
